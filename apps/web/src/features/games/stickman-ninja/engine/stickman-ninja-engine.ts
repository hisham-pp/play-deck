export type NinjaAction =
  'idle' | 'walking' | 'crouching' | 'jumping' | 'throwing' | 'takedown' | 'dead';
...
export function createInitialPlatforms(level: number): Platform[] {
  // Ground
  const platforms: Platform[] = [{ x: 0, y: ARENA_FLOOR_Y, width: ARENA_WIDTH, height: 100 }];
...
export function stepNinjaEngine(
    ...
    .map((s) => {
      ...
      if (nextY >= ARENA_FLOOR_Y || nextX < 0 || nextX > ARENA_WIDTH) {
        return {
          ...
          y: Math.min(ARENA_FLOOR_Y, nextY),
          active: false,
          embedded: true,
        };
      }
  ...
  for (const g of next.guards) {
    ...
    const canSeePlayer =
      !p.isHiding && facingPlayer && dist < GUARD_VISION_DISTANCE && Math.abs(dy) < 65;
