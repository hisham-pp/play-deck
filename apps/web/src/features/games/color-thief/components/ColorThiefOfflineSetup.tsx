...
export interface ColorThiefOfflineSetupProps {
  onStart: (seats: ColorThiefSeat[], settings: ColorThiefRuleSettings) => void;
  onBack: () => void;
}

export function ColorThiefOfflineSetup({ onStart, onBack }: ColorThiefOfflineSetupProps) {
  ...
  useEffect(() => {
    void colorThiefPreferencesRepository.getPreferences().then((prefs) => {
      ...
      setBoardSize(prefs.lastBoardSize);
    });
  }, []);
  ...
  const handleStart = () => {
    const seats: ColorThiefSeat[] = kinds.map((kind, index) => ({
      ...
      displayName:
        index === 0 && player
          ...
          : kind === KIND_BOT
            ? `Bot ${index}`
            : `Player ${index + 1}`,
      avatar: index === 0 ? (player?.avatar ?? '🎨') : kind === KIND_BOT ? '🤖' : '👤',
      type: kind,
    ...
    void colorThiefPreferencesRepository.savePreferences({
      ...
      <Card className="border-amber-500/20 bg-slate-900/80">
        <CardHeader>
          ...
          <p className="mt-1 text-xs text-slate-400">
            Pass the device between human seats, or fill them with bots. The seat order decides the
            colours, and the colours decide the abilities.
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <ul className="space-y-2">
            {kinds.map((kind, index) => {
              const theme = paintTheme(SEAT_COLORS[index % SEAT_COLORS.length]);
              return (
                <li
                  key={index}
            ...
            <div className="flex flex-wrap gap-2">
              {BOARD_SIZES.map((size) => (
                <Button
                  key={size}
                  size="sm"
                  variant={boardSize === size ? 'primary' : 'outline'}
            ...
            <div className="flex flex-wrap gap-2">
              {ROUND_OPTIONS.map((rounds) => (
                <Button
                  key={rounds}
                  size="sm"
                  variant={totalRounds === rounds ? 'primary' : 'outline'}
                  onClick={() => setTotalRounds(rounds)}
                  aria-pressed={totalRounds === rounds}
                >
                  {rounds}
                </Button>
              ))}
