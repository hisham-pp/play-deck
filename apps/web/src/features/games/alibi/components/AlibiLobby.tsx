...
export function AlibiLobby({ roomCode, players, isHost, onAddBot, onRemoveBot, onStart }: Props) {
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <h2 className="text-2xl font-black text-white font-display">Alibi</h2>
        <p className="text-sm text-deck-400">
          Compare stories — one player&apos;s alibi contains deliberate inconsistencies!
        </p>
      </div>
      ...
      {isHost && (
        ...
        >
          {canStart
            ? 'Start Investigation'
            : `Need ${Math.max(0, 4 - players.length)} more detective(s)`}
        </button>
      )}
    </div>
  );
