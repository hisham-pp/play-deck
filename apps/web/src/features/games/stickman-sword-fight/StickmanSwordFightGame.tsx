...
export function StickmanSwordFightGame() {
  ...
  const cycleDifficulty = useCallback(() => {
    const next: Difficulty =
      difficulty === 'easy' ? 'normal' : difficulty === 'normal' ? 'hard' : difficulty === 'hard' ? 'expert' : 'easy';
    setDifficulty(next);
  ...
  const renderFighter = (
    ...
    // Stickman Body
    ctx.strokeStyle = f.state === 'staggered' ? '#eab308' : f.state === 'hit' ? '#ef4444' : '#f8fafc';
    ctx.lineWidth = 4;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ...
    <div className="flex flex-col items-center justify-center p-4 bg-slate-950 text-slate-100 rounded-xl max-w-5xl mx-auto shadow-2xl border border-slate-800">
      ...
      <div className="w-full flex flex-wrap items-center justify-between gap-4 mb-3">
        <div className="flex items-center gap-3">
          ...
          >
            <Badge variant="outline" className="border-cyan-500/50 text-cyan-400 font-mono hover:bg-cyan-950/40">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="text-cyan-400 font-bold flex items-center gap-1">
              {state.p1.name}
              {state.p1.roundsWon > 0 && <span className="text-amber-400">{'★'.repeat(state.p1.roundsWon)}</span>}
            </span>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="text-rose-400 font-bold flex items-center gap-1">
              {state.p2.name}
              {state.p2.roundsWon > 0 && <span className="text-amber-400">{'★'.repeat(state.p2.roundsWon)}</span>}
            </span>
        ...
        {state.status === 'match_over' && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/75 backdrop-blur-sm">
            <div className="text-center p-6 bg-slate-900/90 rounded-2xl border border-amber-500/40 shadow-2xl max-w-sm">
              ...
              <Button onClick={handleRestart} className="w-full bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold">
                Play Again
              </Button>
            </div>
          </div>
