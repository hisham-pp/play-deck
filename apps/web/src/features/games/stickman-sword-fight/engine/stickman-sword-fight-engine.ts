...
export function createInitialFighter(id: 'p1' | 'p2', name: string, x: number, facing: 1 | -1): Fighter {
  return {
    id,
    name,
    x,
...
export function handleFighterInput(
  ...
  // Attack inputs
  if (input.heavySlash && fighter.attackCooldown <= 0 && fighter.stamina >= 25 && fighter.state !== 'slashing' && fighter.state !== 'heavy_slashing' && fighter.state !== 'parrying') {
  ...
  if (input.slash && fighter.attackCooldown <= 0 && fighter.stamina >= 12 && fighter.state !== 'slashing' && fighter.state !== 'heavy_slashing' && fighter.state !== 'parrying') {
  ...
  // Parry input
  if (input.parry && fighter.state !== 'slashing' && fighter.state !== 'heavy_slashing' && fighter.state !== 'dashing') {
  ...
  // Dash input
  if (input.dash && fighter.dashCooldown <= 0 && fighter.stamina >= 18 && fighter.state !== 'dashing') {
  ...
  // Jump input
  if (input.jump && fighter.isGrounded && fighter.state !== 'slashing' && fighter.state !== 'heavy_slashing' && fighter.state !== 'dashing') {
    fighter.vy = -13;
...
export function updateAI(ai: Fighter, player: Fighter, state: SwordFightState, dt: number): void {
  ...
  const diffMultiplier =
    state.difficulty === 'easy' ? 0.4 : state.difficulty === 'normal' ? 0.7 : state.difficulty === 'hard' ? 0.9 : 1.0;

  // React to player attacks (parry or dodge chance)
  if ((player.state === 'slashing' || player.state === 'heavy_slashing') && dist < ATTACK_RANGE + 20) {
    if (ai.state !== 'parrying' && ai.state !== 'dashing' && Math.random() < 0.65 * diffMultiplier) {
...
export function spawnSparks(x: number, y: number, color: string, count: number, state: SwordFightState): void {
...
export function spawnFloatingText(text: string, x: number, y: number, color: string, state: SwordFightState): void {
...
export function checkHitRegistration(attacker: Fighter, defender: Fighter, state: SwordFightState): void {
  ...
  if (dist <= ATTACK_RANGE && correctDirection) {
    ...
    if (defender.state === 'parrying' && defender.parryWindowActive) {
      ...
      attacker.posture = Math.min(attacker.maxPosture, attacker.posture + PARRY_DEFLECT_POSTURE_DAMAGE);
