...
export function createInitialRacingState(
  highScore = 0,
  raceDistance = 400,
): StickmanRacingState {
...
export function stepRacingEngine(
  ...
  if (player.isBoosting && player.nitroGauge > 0 && player.stumbleTimer <= 0) {
    ...
    const targetSpeed = player.stumbleTimer > 0 ? player.baseSpeed * 0.4 : player.baseSpeed + speedBoost;
  ...
  for (const rival of rivals) {
    ...
    const nextHurdle = state.hurdles.find(
      (h) => h.lane === rival.targetLane && h.distanceMeters > rival.distanceMeters && h.distanceMeters - rival.distanceMeters < 12,
    );
