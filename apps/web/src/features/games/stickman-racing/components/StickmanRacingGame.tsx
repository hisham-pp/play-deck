...
export function StickmanRacingGame() {
  ...
  const savedHighScore = usePlayerStore(
    (s) => s.stats.bestScores?.['stickman-racing'] ?? 0,
  );
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-surface-border bg-slate-950 shadow-2xl">
        ...
        <div className="pointer-events-none absolute top-3 left-3 right-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-slate-950/80 backdrop-blur-md px-3 py-2 border border-surface-border">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm font-black text-amber-400">
                {state.player.rank === 1 ? '1ST PLACE' : state.player.rank === 2 ? '2ND PLACE' : '3RD PLACE'}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 rounded-xl bg-slate-950/80 backdrop-blur-md px-4 py-2 border border-surface-border">
            <Zap className={`h-4 w-4 ${state.player.isBoosting ? 'text-amber-400 animate-bounce' : 'text-sky-400'}`} />
          ...
          {/* Nitro Tank Gauge */}
          <div className="flex items-center gap-2 rounded-xl bg-slate-950/80 backdrop-blur-md px-3 py-2 border border-surface-border">
            <Flame className={`h-4 w-4 ${state.player.nitroGauge > 25 ? 'text-amber-400' : 'text-slate-500'}`} />
            <div className="flex flex-col">
            ...
            <span className="text-xs font-bold text-amber-400">{Math.round(state.player.nitroGauge)}%</span>
          </div>
        </div>
        ...
        {state.status === 'ready' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/85 backdrop-blur-md p-6 text-center">
            ...
            <p className="mt-2 max-w-md text-sm text-slate-300">
              Shift between 3 lanes, leap high hurdles, and draft rival runners to charge supersonic nitro boosts!
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-2 text-xs text-slate-400">
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">⬅️ ➡️ A / D: Shift Lanes</span>
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">⬆️ Space: Hurdle Jump</span>
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">⚡ Shift: Nitro Boost</span>
            </div>
            <button
              onClick={handleStart}
        ...
        {state.status === 'finished' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/90 backdrop-blur-md p-6 text-center">
            <Trophy className={`h-12 w-12 ${state.player.rank === 1 ? 'text-amber-400 animate-bounce' : 'text-slate-400'}`} />
            ...
            <p className="text-sm text-slate-300">Finished {state.raceDistance}m in {state.player.finishTime} seconds.</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 w-full max-w-xs text-left text-xs">
              ...
              <div className="rounded-lg bg-surface-base p-2.5 border border-surface-border">
                ...
                <p className="text-lg font-bold text-purple-400">{state.stats.draftSeconds.toFixed(1)}s</p>
              </div>
            </div>
