...
export function StickmanShooterGame() {
  ...
  const savedHighScore = usePlayerStore(
    (s) => s.stats.bestScores?.['stickman-shooter'] ?? 0,
  );
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-surface-border bg-slate-950 shadow-2xl">
        ...
        <div className="pointer-events-none absolute top-3 left-3 right-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-slate-950/80 backdrop-blur-md px-3 py-2 border border-surface-border">
            <Shield className={`h-4 w-4 ${state.player.health <= 30 ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`} />
          ...
          <div className="flex items-center gap-2 rounded-xl bg-slate-950/80 backdrop-blur-md px-3 py-2 border border-surface-border">
            <div className="flex flex-col items-end">
              ...
              <span className={`text-base font-black tracking-wider ${state.player.ammo === 0 ? 'text-red-500 animate-pulse' : 'text-amber-400'}`}>
                {state.player.isReloading ? 'RELOADING...' : `${state.player.ammo} / ${state.player.maxAmmo}`}
              </span>
            </div>
            <Zap className={`h-4 w-4 ${state.player.ammo === 0 ? 'text-red-500' : 'text-amber-400'}`} />
          </div>
        </div>
        ...
        {state.status === 'ready' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/85 backdrop-blur-md p-6 text-center">
            ...
            <p className="mt-2 max-w-md text-sm text-slate-300">
              Duck behind cover, time your breaches, dodge enemy sniper fire, and eliminate hostiles with precision headshots.
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-2 text-xs text-slate-400">
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">🎯 Mouse: Aim &amp; Fire</span>
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">🛡️ S / Space: Duck / Peek</span>
              <span className="rounded bg-surface-raised px-2 py-1 border border-surface-border">⚡ R: Reload</span>
            </div>
            <button
              onClick={handleStart}
        ...
        {state.status === 'game_over' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/90 backdrop-blur-md p-6 text-center">
            ...
            <p className="text-sm text-slate-300">Overrun by hostile marksmen on Wave {state.wave}.</p>
