...
import { LootDashVoiceDock } from '@/features/voice/components/LootDashVoiceDock';
import { useLootDashMultiplayerStore } from '@/stores/loot-dash-multiplayer.store';

interface LootDashRoomLobbyProps {
  onStartMatch: () => void;
  onBackToLobby: () => void;
}

export function LootDashRoomLobby({ onStartMatch, onBackToLobby }: LootDashRoomLobbyProps) {
  const { roomCode, players, isHost, addBot, removeBot, leaveRoom } = useLootDashMultiplayerStore();
  ...
  const handleCopyLink = async () => {
    ...
    const url = `${window.location.origin}/play/loot-dash?room=${roomCode}`;
    ...
    <div className="w-full max-w-[960px] mx-auto bg-slate-900/90 border border-slate-700/60 rounded-2xl p-6 shadow-2xl backdrop-blur-md text-slate-100 flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-800">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-100 mt-1">Loot Dash Scavenger Lounge</h1>
        </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
            REGISTERED RUNNERS ({players.length}/6)
          </span>
          {host && players.length < 6 && (
            <button
              type="button"
              onClick={addBot}
              className="px-3 py-1 rounded-lg text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-amber-400 border border-slate-700 transition-all cursor-pointer"
            >
              + Add AI Bot
            </button>
          )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-slate-400">
                    {p.isBot ? 'Autonomous Sprinter' : 'Live Player'}
                  </span>
                </div>
              </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-slate-800">
        <button
          type="button"
          onClick={handleLeave}
          className="px-4 py-2 rounded-xl text-xs font-bold text-slate-400 hover:text-slate-200 bg-slate-800/60 border border-slate-700/60 transition-all cursor-pointer"
        >
          Leave Room
        </button>
        ...
        {host ? (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer ${
              players.length >= 2
                ? 'bg-linear-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 shadow-lg shadow-amber-500/20'
          ...
          <span className="text-xs text-amber-400/80 italic">
            Waiting for host to launch the dash...
          </span>
        )}
      </div>
    </div>
