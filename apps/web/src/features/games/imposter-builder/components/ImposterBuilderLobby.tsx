...
export function ImposterBuilderLobby({
  roomCode,
  players,
  isHost,
  onAddBot,
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <h2 className="text-2xl font-black text-white font-display">Imposter Builder</h2>
        <p className="text-sm text-deck-400">
          Build the same thing — but one of you got different instructions!
        </p>
      </div>
      ...
      {isHost && (
        ...
        >
          {canStart ? 'Start Building!' : `Need ${Math.max(0, 4 - players.length)} more builder(s)`}
        </button>
      )}
    </div>
  );
