...
import { GAME_ID, MAX_SEATS, MIN_SEATS } from '../engine/giant-constants';

interface GiantRoomLobbyProps {
  onStartGame: () => void;
  onLeave: () => void;
}

export function GiantRoomLobby({ onStartGame, onLeave }: GiantRoomLobbyProps) {
  const { roomCode, isHost, seats, addBot, removeBot, setStatus, leaveRoom } =
    useGiantMultiplayerStore();
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 py-4">
      <Card className="border-[#1e293b] bg-[#0b101d]">
        ...
        <CardContent className="flex flex-col items-stretch justify-between gap-4 border-t border-[#1e293b] pt-4 sm:flex-row sm:items-center">
          ...
          {roomCode && <ShareRoomLink roomCode={roomCode} gameId={GAME_ID} />}
        </CardContent>
      </Card>
          ...
          <div className="mt-6 flex flex-col items-center justify-between gap-4 border-t border-[#1e293b] pt-4 sm:flex-row">
            <span className="text-xs text-slate-400">
              {seats.length < MIN_SEATS
                ? `Need at least ${MIN_SEATS} in the crew (add AI burglars or invite friends).`
                : 'Everyone in position. Nobody breathe.'}
            </span>
            ...
            {isHost() ? (
              ...
              <span className="text-sm italic text-slate-400">
                Waiting for the host to begin the heist…
              </span>
            )}
          </div>
        </CardContent>
