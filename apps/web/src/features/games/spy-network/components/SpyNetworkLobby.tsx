...
import type { SpyNetworkPlayer } from '../types/spy-network.types';
...
export function SpyNetworkLobby({
  roomCode,
  players,
  isHost,
  onAddBot,
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <h2 className="text-2xl font-black text-white font-display">Spy Network</h2>
        <p className="text-sm text-deck-400">Find the spy before they find the location!</p>
      </div>
      ...
      {isHost && (
        ...
        >
          {canStart ? 'Start Mission' : `Need ${Math.max(0, 4 - players.length)} more agent(s)`}
        </button>
      )}
    </div>
  );
