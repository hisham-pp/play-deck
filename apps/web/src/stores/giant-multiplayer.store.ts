...
import {
  ...
  MAX_SEATS,
  SEAT_COLORS,
} from '@/features/games/dont-wake-the-giant/engine/giant-constants';
import { GN_MSG } from '@/features/games/dont-wake-the-giant/multiplayer/giant-protocol';
import type { GiantSeat } from '@/features/games/dont-wake-the-giant/types/giant.types';
...
const CHANNEL_NAMESPACE = 'dont-wake-the-giant';
...
interface GiantMultiplayerState {
  roomCode: string | null;
  hostId: string | null;
  localPlayerId: string | null;
  status: 'idle' | 'lobby' | 'playing' | 'error';
  /** Every seat in the heist: humans reconciled from presence, plus any bots. */
  seats: GiantSeat[];
  /** Seated humans missing from presence; their thief stops moving, not existing. */
  ...
  /** Host pushes the agreed roster so every client seats the crew identically. */
  publishSeats: (seats: GiantSeat[]) => void;
  adoptSeats: (seats: GiantSeat[]) => void;
  setStatus: (status: GiantMultiplayerState['status']) => void;
  isHost: () => boolean;
}

function toPresence(seat: GiantSeat, hostId: string | null): PlayerPresence {
  return {
    playerId: seat.id,
...
function humanSeat(presence: PlayerPresence, seatIndex: number): GiantSeat {
  return {
...
export const useGiantMultiplayerStore = create<GiantMultiplayerState>((set, get) => {
  /** Bots are host-owned and never appear in presence, so they are tracked apart. */
  let bots: GiantSeat[] = [];

  function rebuildFromSeats(): GiantSeat[] {
  ...
  function attachPresence(transport: SupabaseTransportService) {
    transport.onPresence((present) => {
      ...
      // Mid-heist the seating is frozen: a thief's haul has to survive a dropped
      // connection, so a missing human is marked absent rather than removed.
      if (status === 'playing') {
        set({
  ...
  return {
    ...
    joinRoomByCode: async (code, identity) => {
      try {
        ...
        if (!room || room.gameId !== GAME_ID) {
          set({ error: "That room is not a Don't Wake the Giant room", status: STATUS_ERROR });
          return false;
        }
    ...
    publishSeats: (seats) => {
      ...
      transport.send(GN_MSG.SEATS, { seats }, hostId);
    },
