...
import {
  ...
  MAX_SEATS,
  SEAT_COLORS,
} from '@/features/games/shadow-tag/engine/shadow-tag-constants';
import type { ShadowTagSeat } from '@/features/games/shadow-tag/types/shadow-tag.types';
...
const CHANNEL_NAMESPACE = 'shadow-tag';
...
interface ShadowTagMultiplayerState {
  roomCode: string | null;
  hostId: string | null;
  localPlayerId: string | null;
  status: 'idle' | 'lobby' | 'playing' | 'error';
  /** Every seat in the round: humans reconciled from presence, plus any bots. */
  seats: ShadowTagSeat[];
  /** Seated humans missing from presence; their shadow stops moving, not existing. */
  ...
  publishSeats: (seats: ShadowTagSeat[]) => void;
  adoptSeats: (seats: ShadowTagSeat[]) => void;
  setStatus: (status: ShadowTagMultiplayerState['status']) => void;
  isHost: () => boolean;
}

function toPresence(seat: ShadowTagSeat, hostId: string | null): PlayerPresence {
  return {
    playerId: seat.id,
...
function humanSeat(presence: PlayerPresence, seatIndex: number): ShadowTagSeat {
  return {
...
export const useShadowTagMultiplayerStore = create<ShadowTagMultiplayerState>((set, get) => {
  /** Bots are host-owned and never appear in presence, so they are tracked apart. */
  let bots: ShadowTagSeat[] = [];

  function rebuildFromSeats(): ShadowTagSeat[] {
  ...
  function attachPresence(transport: SupabaseTransportService) {
    transport.onPresence((present) => {
      ...
      // Mid-round the seating is frozen: a runner's score and standing have to
      // survive a dropped connection, so a missing human is marked absent.
      if (status === 'playing') {
        set({
  ...
  return {
    ...
    joinRoomByCode: async (code, identity) => {
      try {
        ...
        if (!room || room.gameId !== GAME_ID) {
          set({ error: 'That room is not a Shadow Tag room', status: STATUS_ERROR });
          return false;
        }
    ...
    publishSeats: (seats) => {
      ...
      transport.send('ST_SEATS', { seats }, hostId);
    },
